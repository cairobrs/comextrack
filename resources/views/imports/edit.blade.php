                    ...
                    <form method="POST" action="{{ route('imports.update', $import) }}">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <select id="responsavel_interno_id" name="responsavel_interno_id" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                                    ...
                                    @foreach($users as $user)
                                        <option value="{{ $user->id }}" {{ old('responsavel_interno_id', $import->responsavel_interno_id) == $user->id ? 'selected' : '' }}>
                                            {{ $user->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <x-text-input id="porto_origem" class="block mt-1 w-full" type="text" name="porto_origem" :value="old('porto_origem', $import->porto_origem)" />
                                <x-input-error :messages="$errors->get('porto_origem')" class="mt-2" />
                            </div>

                            <div>
                                <x-input-label for="porto_destino" :value="__('Porto de Destino')" />
                                <x-text-input id="porto_destino" class="block mt-1 w-full" type="text" name="porto_destino" :value="old('porto_destino', $import->porto_destino)" />
                            ...
                            <div>
                                ...
                                <x-text-input id="taxa_cambio" class="block mt-1 w-full {{ old('moeda', $import->moeda) == 'BRL' ? 'bg-gray-100' : '' }}" type="text" name="taxa_cambio_display" :value="old('taxa_cambio') ? number_format((float)old('taxa_cambio'), 4, ',', '.') : ($import->taxa_cambio ? number_format((float)$import->taxa_cambio, 4, ',', '.') : (old('moeda', $import->moeda) == 'BRL' ? '1,0000' : ''))" placeholder="Ex.: 5,40 se 1 USD = 5,40 BRL" {{ old('moeda', $import->moeda) == 'BRL' ? 'disabled' : '' }} />
                                <input type="hidden" id="taxa_cambio_hidden" name="taxa_cambio" value="{{ old('taxa_cambio', $import->taxa_cambio ?? (old('moeda', $import->moeda) == 'BRL' ? '1.0000' : '')) }}" />
            ...
            if (taxaCambioInput && taxaCambioHidden && moedaSelect) {
                ...
                window.handleMoedaChange = function() {
                    ...
                    if (moeda === 'BRL') {
                        ...
                        if (!taxaCambioInput.value || taxaCambioInput.value === '1,0000') {
